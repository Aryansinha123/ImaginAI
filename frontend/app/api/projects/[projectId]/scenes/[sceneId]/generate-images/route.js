...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // Call the Python backend to generate images
    const apiRes = await axios.post("http://127.0.0.1:8000/generate-scene-images", {
    ...
    const imageFilenames = apiRes.data.images || [];
    const imageFilename = apiRes.data.image || null;

    // Update the scene in the database with the new images
    const updated = await db.collection("scenes").findOneAndUpdate(
      { _id: new ObjectId(sceneId), project_id: new ObjectId(projectId) },
      { $set: { images: imageFilenames, image: imageFilename } },
