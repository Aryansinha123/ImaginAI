      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        {activeProject ? (
          <div className="mb-6 px-2 shrink-0 relative">
            <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-500 font-semibold block mb-1.5">
              Active Universe
            </span>
            <button
              onClick={() => setShowProjectSelect(!showProjectSelect)}
              className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-2xl text-left hover:border-zinc-700 transition-all cursor-pointer"
            >
          ...
          <div className="mb-6 px-2 shrink-0">
            ...
            >
              <span className="text-sm font-semibold text-zinc-400">Choose Project...</span>
        ...
        {showProjectSelect && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 mb-6 flex flex-col max-h-64 overflow-hidden shrink-0 space-y-3">
            ...
            {isCreating && (
              <form onSubmit={handleCreate} className="flex items-center gap-2 shrink-0">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="p-1.5 bg-white text-black hover:bg-zinc-200 rounded-lg transition-colors disabled:bg-zinc-700 cursor-pointer animate-pulse"
                >
            ...
            <div className="flex-1 overflow-y-auto space-y-1 pr-1 scrollbar-thin">
              {projects.length === 0 ? (
                <p className="text-[11px] text-zinc-500 text-center py-4">No universes created yet.</p>
              ) : (
        ...
        {activeProject && !showProjectSelect && (
          <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 shrink-0 py-2 border-t border-zinc-900/50 scrollbar-thin">
            <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-500 font-semibold block px-2 mb-2">
              Workspace
            </span>
            {navItems.map((item) => {
        ...
        <div className="border-t border-zinc-900 pt-5 flex items-center justify-between px-2 shrink-0">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <span className="text-[10px] text-zinc-500 font-mono truncate">Writer</span>
            </div>
          </div>
          <button
            onClick={logout}
